...
from data.transforms.pose import flip_joints_3d, get_affine_transform, affine_transform
...
class SimplePoseDefaultValTransform(object):
    ...
    def __call__(self, src, label, img_path):
        ...
        img = cv2.warpAffine(np.asarray(src), trans, (int(w), int(h)), flags=cv2.INTER_LINEAR)
