...
class VOCSegmentationPaper(SegmentationDataset):
    """Pascal VOC Semantic Segmentation Dataset.

    Parameters
    ----------
    root : string
        Path to VOCdevkit folder. Default is '$(HOME)/.torch/datasets/voc'
    split: string
        'train', 'val' or 'test'
    ...
    def __init__(self, root=os.path.expanduser('~/.torch/datasets/voc'),
                 split='train', mode=None, transform=None, **kwargs):
        super(VOCSegmentationPaper, self).__init__(root, split, mode, transform, **kwargs)
        _voc_root = os.path.join(root, self.BASE_DIR)
        # train/val/test splits are pre-cut
        _splits_dir = os.path.join(_voc_root, 'list')
        if split == 'train':
...
if __name__ == '__main__':
    data = VOCSegmentationPaper(mode='val')
    print(data[0][1].max())
