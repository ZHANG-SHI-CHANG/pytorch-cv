...
class COCOSegmentation(SegmentationDataset):
    ...
    ----------
    root : string
        Path to VOCdevkit folder. Default is '$(HOME)/.torch/datasplits/ade'
    split: string
        'train', 'val' or 'test'
    ...
    >>> ])
    >>> # Create Dataset
    >>> trainset = data.COCOSegmentation(split='train', transform=input_transform)
    >>> # Create Training Loader
    >>> train_data = data.DataLoader(
    >>>     trainset, 4, shuffle=True, last_batch=True,
    >>>     num_workers=4)
    """
    ...
    def __init__(self, root=os.path.expanduser('~/.torch/datasets/coco'),
