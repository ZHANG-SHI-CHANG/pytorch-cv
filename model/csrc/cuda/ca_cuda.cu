...
template <typename T>
__global__ void ca_map_backward_kernel_w(const T *dout, const T *weight, const T *g, T *dw,
                                         int num, int chn, int height, int width) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int sp = height * width;
...
template<typename T>
__global__ void ca_map_backward_kernel_g(const T *dout, const T *weight, const T *g, T *dg,
                                         int num, int chn, int height, int width) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int sp = height * width;
...
at::Tensor ca_forward_cuda(const at::Tensor &t, const at::Tensor &f) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(t.type(), "ca_forward", [&] {
...
std::tuple<at::Tensor, at::Tensor> ca_backward_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(t.type(), "ca_backward_kernel_t", [&] {
        ca_backward_kernel_t < scalar_t ><<<blocks, threads, 0, stream>>> (
        dw.contiguous().data<scalar_t>(), t.contiguous().data<scalar_t>(),
        f.contiguous().data<scalar_t>(), dt.contiguous().data<scalar_t>(),
        n, c, h, w);
    });

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(t.type(), "ca_backward_kernel_f", [&] {
        ca_backward_kernel_f < scalar_t ><<<blocks, threads, 0, stream>>> (
        dw.contiguous().data<scalar_t>(), t.contiguous().data<scalar_t>(),
        f.contiguous().data<scalar_t>(), df.contiguous().data<scalar_t>(),
        n, c, h, w);
    });
...
at::Tensor ca_map_forward_cuda(const at::Tensor &weight, const at::Tensor &g) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(g.type(), "ca_map_forward_kernel", [&] {
...
std::tuple<at::Tensor, at::Tensor> ca_map_backward_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(g.type(), "ca_map_backward_kernel_w", [&] {
        ca_map_backward_kernel_w < scalar_t ><<<blocks, threads, 0, stream>>> (
        dout.contiguous().data<scalar_t>(), weight.contiguous().data<scalar_t>(),
        g.contiguous().data<scalar_t>(), dw.contiguous().data<scalar_t>(), n, c, h, w);
    });
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(g.type(), "ca_map_backward_kernel_w", [&] {
        ...
        dout.contiguous().data<scalar_t>(), weight.contiguous().data<scalar_t>(),
        g.contiguous().data<scalar_t>(), dg.contiguous().data<scalar_t>(), n, c, h, w);
    });
