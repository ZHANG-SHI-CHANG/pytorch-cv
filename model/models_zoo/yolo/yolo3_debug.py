...
class YOLOV3(nn.Module):
    ...
    def __init__(self, stages, transitions, yolo_blocks, yolo_outputs, anchors, classes, nms_thresh=0.45, nms_topk=400,
                 post_nms=100, pos_iou_thresh=1.0, ignore_iou_thresh=0.7, **kwargs):
...
def get_yolov3(name, stages, out_channels, block_channels, filters, anchors, strides, classes, dataset,
               pretrained=False, root=os.path.join(os.path.expanduser('~'), '.torch', 'models'),
               alloc_size=(128, 128), norm_layer=nn.BatchNorm2d, norm_kwargs=None, **kwargs):
    """Get YOLOV3 models.
    Parameters
    ----------
    name : str or None
    ...
    yolo_outputs = list()
    for i, stage, out_channel, block_channel, channel, anchor, stride in zip(
            range(len(stages)), stages, out_channels, block_channels, filters, anchors[::-1], strides[::-1]):
        ...
        if i > 0:
            transitions.append(_conv2d(out_channel, channel, 1, 0, 1, norm_layer=norm_layer,
                                       norm_kwargs=norm_kwargs))

    stages_ = nn.ModuleList(stages_)
    yolo_blocks = nn.ModuleList(yolo_blocks)
    transitions = nn.ModuleList(transitions)
    yolo_outputs = nn.Sequential(*yolo_outputs)
    net = YOLOV3(stages_, transitions, yolo_blocks, yolo_outputs, anchors, classes=classes, **kwargs)
    # if pretrained:
