...
def gluon2torch(name, gluon_path, torch_path, base=False):
    name = name.lower()
    if base:
        torch_model = model_zoo.get_model(name, pretrained=True, pretrained_base=False)
...
if __name__ == '__main__':
    ...
    parse.add_argument('--name', type=str, default='CIFAR_ResNeXt29_16x64d', help='name of the model')
    ...
    gluon2torch(config.name, config.gluon_path, config.torch_path, config.base)
